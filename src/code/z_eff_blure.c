...
void EffectBlure_AddSpace(EffectBlure* this) {
    ...
    if (this != NULL) {
        numElements = this->numElements;
        if (numElements >= 16) {
            PRINTF("Blure space addition processing: Table over %d\n", numElements);
            return;
        }
...
void EffectBlure_DrawElemNoInterpolation(EffectBlure* this, EffectBlureElement* elem, s32 index,
    ...
    if (vtx == NULL) {
        PRINTF("z_eff_blure.c::SQ_NoInterpolate_disp() Vertices cannot be secured.\n");
    } else {
        vtx[0].v = baseVtx;
        vtx[1].v = baseVtx;
        vtx[2].v = baseVtx;
...
void EffectBlure_DrawElemHermiteInterpolation(EffectBlure* this, EffectBlureElement* elem, s32 index,
    ...
    if (vtx == NULL) {
        PRINTF("z_eff_blure.c::SQ_HermiteInterpolate_disp() Vertices cannot be secured.\n");
    } else {
        ...
        for (i = 0; i < this->numElements - 1; i++) {
            ...
            if (this->flags & 4) {
                ...
                if (fabsf(scale) > 0.0005f) {
                    ...
                    if (mtx == NULL) {
                        PRINTF("EffectBlureInfo2_disp_makeDisplayList()Forced termination because a matrix cannot be taken\n");
                        break;
                    }
