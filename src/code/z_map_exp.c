...
void Map_SetPaletteData(PlayState* play, s16 room) {
    ...
    PRINTF("ＰＡＬＥＴＥPALETE SetRoom_Inf[%d][4]=%x  ( map_palete_no = %d )\n", paletteIndex, room,
...
void Map_InitData(PlayState* play, s16 room) {
    ...
    switch (play->sceneId) {
        ...
        case SCENE_SHADOW_TEMPLE_BOSS:
            PRINTF(VT_FGCOL(YELLOW));
            PRINTF("Deku Tree Dungeon MAP Texture DMAＤＭＡ(%x) scene_id_offset=%d  VREG(30)=%d\n", room, mapIndex,
                   VREG(30));
            PRINTF(VT_RST);
            ...
            PRINTF("ＭＡＰ MAP Individual Floor ON Check\n"); // "MAP Individual Floor ON Check"
            break;
    }
}
...
void Map_Init(PlayState* play) {
    ...
    PRINTF("\n\n\nＭＡＰ texture initialization scene_data_ID=%d mapSegment=%x   scene_data_ID=%d\nmapSegment=%x\n\n", play->sceneId,
        ...
        switch (play->sceneId) {
            ...
            case SCENE_ICE_CAVERN:
                ...
                if (interfaceCtx->mapRoomNum != sLastRoomNum) {
                    PRINTF("Current floor = %d Current room = %x Number of rooms = %d＝%d\n", floor, interfaceCtx->mapRoomNum,
