...
// sets alpha for (disabled) buttons
...
void Interface_DrawDPad(GlobalContext* globalCtx) {
    InterfaceContext* interfaceCtx = &globalCtx->interfaceCtx;
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_parameter.c", 2795);
    ...
    // up: weapon swap
    if (GetDPadItem(globalCtx) < 0xF0) {
        gDPPipeSync(OVERLAY_DISP++);
        gDPLoadTextureBlock(OVERLAY_DISP++, interfaceCtx->iconItemSegment + 0x6000, G_IM_FMT_RGBA, G_IM_SIZ_32b, 32, 32, 0, G_TX_NOMIRROR | G_TX_WRAP,
                        G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

        gSPTextureRectangle(OVERLAY_DISP++, tlx, tly - uoffset, brx, bry - uoffset, G_TX_RENDERTILE, 0, 0, 2 << 10, 2 << 10);
    }
