...
void EnWiz_Init(Actor* thisx, PlayState* play) {
    ...
    this->type = (thisx->params & 0x7F00) >> 8;
...
void EnWiz_Destroy(Actor* thisx, PlayState* play2) {
    ...
    if (Actor_FindNearby(play, &this->actor, ACTOR_EN_WIZ, ACTORCAT_ENEMY, 8000.0f) == NULL) {
        func_800F5B58();
    }
...
// Appear
void EnWiz_SetupAppear(EnWiz* this) {
    if ((this->actor.params & 0x8000) && (!this->minibossMusicStarted)) {
        func_800F5ACC(NA_BGM_MINI_BOSS);
        this->minibossMusicStarted = true;
    }    
