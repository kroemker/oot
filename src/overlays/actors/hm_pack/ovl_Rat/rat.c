...
void Rat_Wander(Rat* this, PlayState* play);
...
void Rat_SetupWander(Rat* this, PlayState* play) {
    s16 angle = Rand_S16Offset(DEG_TO_BINANG(-180.0f), DEG_TO_BINANG(180.0f));
    this->targetPos.x = (Math_CosS(angle) * 12.0f) + this->actor.home.pos.x;
    this->targetPos.z = (Math_SinS(angle) * 12.0f) + this->actor.home.pos.z;
    this->wanderTimer = Rand_S16Offset(60, 20);
    Animation_MorphToLoop(&this->skelAnime, &gRatSkelRunAnim, -4.0f);
    this->actionFunc = Rat_Wander;
}
...
void Rat_Idle(Rat* this, PlayState* play) {
    ...
    else if (DECR(this->wanderTimer) == 0) {
        Rat_SetupWander(this, play);
    }
}
...
void Rat_Wander(Rat* this, PlayState* play) {
    ...
    Rat_RotateTowardPoint(this, &this->targetPos, DEG_TO_BINANG(30.0f));
    Math_SmoothStepToF(&this->actor.speed, 1.5f, 0.1f, 1.0f, 0.0f);
    if (Actor_WorldDistXZToPoint(&player->actor, &this->targetPos) < 5.0f) {
