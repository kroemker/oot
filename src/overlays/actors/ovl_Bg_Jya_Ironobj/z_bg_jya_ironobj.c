...
typedef void (*BgJyaIronobjIkFunc)(BgJyaIronobj*, GlobalContext*, EnIk*, s32);
...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* this, GlobalContext* globalCtx, EnIk* enIk, s32 hitByPlayer);
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* this, GlobalContext* globalCtx, EnIk* enIk, s32 hitByPlayer);
...
void BgJyaIronobj_SpawnPillarParticles(BgJyaIronobj* this, GlobalContext* globalCtx, EnIk* enIk, s32 hitByPlayer) {
    ...
    rotY = hitByPlayer 
            ? Actor_WorldYawTowardActor(&this->dyna.actor, &(GET_PLAYER(globalCtx)->actor)) 
...
void BgJyaIronobj_SpawnThoneParticles(BgJyaIronobj* this, GlobalContext* globalCtx, EnIk* enIk, s32 hitByPlayer) {
    ...
    for (j = 0; j < 32; j++) {
        ...
        vel.y = Rand_ZeroOne() * 8.0f - 3.0f;
        vel.z = 2.0f * (Rand_ZeroOne() * 6.0f - 3.0f) + (Rand_ZeroOne() * coss * 8.0f * temp_f22);
        EffectSsKakera_Spawn(globalCtx, &pos, &vel, &pos, -350, unkArg5, D_80899530[j & 7], 4, 0, D_80899510[j & 7], 0, 5,
                             D_80899520[j & 7], -1, OBJECT_JYA_IRON, gObjectJyaIronDL_000880);
        if (Rand_ZeroOne() < 0.26f) {
            func_80033480(globalCtx, &pos, 200.0f, 1, D_80899510[j & 7] * 4 + 60, D_80899510[j & 7] * 4 + 80, 1);
        }
    }
}
...
void func_808992E8(BgJyaIronobj* this, GlobalContext* globalCtx) {
    ...
    if (this->colCylinder.base.acFlags & AC_HIT) {
        ...
        if ((actor != NULL && actor->id == ACTOR_EN_IK) || (this->colCylinder.info.acHitInfo->toucher.dmgFlags & 0x02000100)) {
            particleFunc[this->dyna.actor.params & 1](this, globalCtx, (EnIk*)actor, actor->id != ACTOR_EN_IK);
