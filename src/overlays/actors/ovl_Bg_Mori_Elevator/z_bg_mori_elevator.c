...
void BgMoriElevator_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->moriTexObjectSlot < 0) {
        Actor_Kill(thisx);
        PRINTF("Error : Forest Temple obj elevator Bank Danger!！(%s %d)\n", "../z_bg_mori_elevator.c", 277);
        return;
    }
#endif
    ...
    switch (sIsSpawned) {
        case false:
            PRINTF("Forest Temple elevator CT\n");
            sIsSpawned = true;
...
void BgMoriElevator_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_172 == 0) {
        PRINTF("Forest Temple elevator DT\n");
    ...
    if (Object_IsLoaded(&play->objectCtx, this->moriTexObjectSlot)) {
        if (Flags_GetSwitch(play, this->dyna.actor.params & 0x3F)) {
            if (play->roomCtx.curRoom.num == 2) {
                ...
                PRINTF("Error : Error: Forest Temple obj elevator Room setting is dangerous(%s %d)\n", "../z_bg_mori_elevator.c", 371);
            }
        } else {
            BgMoriElevator_SetupSetPosition(this);
        }
