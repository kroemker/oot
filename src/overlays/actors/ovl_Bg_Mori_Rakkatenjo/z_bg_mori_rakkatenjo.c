...
void BgMoriRakkatenjo_Init(Actor* thisx, PlayState* play) {
...
#if OOT_DEBUG
    PRINTF("Forest Temple obj. Falling Ceiling (home posY %f)\n", this->dyna.actor.home.pos.y);
    if ((fabsf(1991.0f - this->dyna.actor.home.pos.x) > 0.001f) ||
        ...
        PRINTF("Warning : The set position has been changed. Let's fix the program.\n");
    }
    if (this->dyna.actor.home.rot.y != 0x8000) {
        PRINTF("Warning : The set Angle has changed. Let's fix the program.\n");
    }
#endif

    this->moriTexObjectSlot = Object_GetSlot(&play->objectCtx, OBJECT_MORI_TEX);
    if (this->moriTexObjectSlot < 0) {
        PRINTF("Error : Forest Temple obj Falling Ceiling Bank Danger!！(%s %d)\n", "../z_bg_mori_rakkatenjo.c", 205);
