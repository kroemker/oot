...
void EnFish_Dropped_Fall(EnFish* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        ...
        PRINTF("BG missing? Running Actor_delete(%s %d)\n", "../z_en_sakana.c", 822);
        PRINTF(VT_RST);
...
void EnFish_UpdateCutscene(EnFish* this, PlayState* play) {
    ...
    if (cue == NULL) {
        PRINTF("Warning : dousa 3 Warning : DEMO ended without dousa (action) 3 termination being called(%s %d)(arg_data 0x%04x)\n", "../z_en_sakana.c", 1169,
               this->actor.params);
    ...
    switch (cue->id) {
        ...
        case 2:
            ...
            break;
        case 3:
            PRINTF("DEMO fish termination\n");
            EnFish_ClearCutsceneData(this);
            Actor_Kill(&this->actor);
            return;
        default:
            PRINTF("Improper DEMO action(%s %d)(arg_data 0x%04x)\n", "../z_en_sakana.c", 1200, this->actor.params);
            break;
    }
