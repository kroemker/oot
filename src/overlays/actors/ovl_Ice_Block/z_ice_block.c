...
void IceBlock_Init(Actor* thisx, PlayState* play);
void IceBlock_Destroy(Actor* thisx, PlayState* play);
void IceBlock_Update(Actor* thisx, PlayState* play);
void IceBlock_Draw(Actor* thisx, PlayState* play);
...
void spawnDust(IceBlock* this, PlayState* play) {
...
void IceBlock_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->dyna.actor.floorPoly != 0) {
        u32 cspeed = SurfaceType_GetConveyorSpeed(&play->colCtx, this->dyna.actor.floorPoly, this->dyna.actor.floorBgId);
        u32 dir = SurfaceType_GetConveyorDirection(&play->colCtx, this->dyna.actor.floorPoly, this->dyna.actor.floorBgId);

        this->dyna.actor.world.rot.y = dir << 10;
        this->dyna.actor.speed = (float)cspeed;
    ...
    if (this->status == 0) {
        SfxSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.world.pos, 0x3C, NA_SE_EV_ICE_FREEZE);
        this->status = 1;
    }
...
void IceBlock_Draw(Actor* thisx, PlayState* play) {
    IceBlock* this = THIS;
    Gfx_DrawDListOpa(play, gIceBlockDL);
}
