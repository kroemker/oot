...
ActorProfile Transform_Octorok_Profile = {
...
void TransformOctorok_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && !(this->actor.bgCheckFlags & BGCHECKFLAG_WATER) && this->actor.depthInWater < -5.0f) {
        Enemy_StartFinishingBlow(play, &this->actor);
        Actor_Kill(&this->actor);
    }
}
...
void TransformOctorok_Action_JumpOutOfWater(TransformOctorok* this, PlayState* play) {
    if (SkelAnime_Update(&this->skelAnime) || (!(this->actor.bgCheckFlags & BGCHECKFLAG_WATER) && (this->actor.depthInWater < -5.0f))) {
        TransformOctorok_SpawnSplash(this, play);
