...
extern CollisionHeader gVineSpotCol_collisionHeader;

void VineSpot_Init(Actor* thisx, GlobalContext* globalCtx);
void VineSpot_Destroy(Actor* thisx, GlobalContext* globalCtx);
void VineSpot_Update(Actor* thisx, GlobalContext* globalCtx);
void VineSpot_Draw(Actor* thisx, GlobalContext* globalCtx);
...
void VineSpot_SpawnDust(VineSpot* this, GlobalContext* globalCtx) {
    f32 sign[] = {-1.0f, 1.0f};
    Vec3f* vinePos;
    Vec3f dustPos;
    Vec3f dustVel;
...
void VineSpot_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    CollisionHeader_GetVirtual(&gVineSpotCol_collisionHeader, &colHeader);
...
void VineSpot_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->lifetime < 50 || this->lifetime > maxLifetime) {
        VineSpot_SpawnDust(this, globalCtx);
    }
...
void VineSpot_Draw(Actor* thisx, GlobalContext* globalCtx) {
    Gfx_DrawDListOpa(globalCtx, gVineSpotDL);
}
